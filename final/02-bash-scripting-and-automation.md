...
Strings and numbers use **different operators**: `=` for text, `-eq` for numbers. That catches everyone out. And `!` means "not", which is more familiar — `[ ! -f "Dockerfile" ]` reads "if a Dockerfile does **not** exist".
...
**ASK** <br>
Compare that to `forEach(item => ...)` or a `map`. What's genuinely different? <br>
**ANSWER** <br>
...
We also have space for functions as well. 
...
This would still need a shebang but it'd live as part of a wider script. 
...
**Part B** <br>
3. Write a script called `create` which contains a `for` loop creating five files named `test1.txt` through `test5.txt`

Remember to run: `chmod +x <script-name>`
...
I have another challenge for you. I want you to write a bash script which does a NASA rocket launch countdown. 

This is hard so I'm going to give you some syntax we've not seen yet. 

There's a command called `seq` which can take up to three arguments. 

*(Run from `terminal`)*
- `seq 5` # Counts from 1 to the first argument
- `seq 5 15`  # Counts from the first argument to the second argument
- `seq 5 2 15` # Counts from the first argument to the third argument in increments of the second argument 

Additionally if you want to do arithmetic in bash, that happens inside two pairs of parenthesies. 

- `echo $((1+12))` # It calculates and returns the value back to us <br>

We need to add the dollar symbol at the front, if I run `echo ((1+1))` we get an error and that's because the parenthesies and 1+1 is just an evaluation. The dollar symbol stores the value as we would a variable, which we can then echo.

- `SLIDE ACROSS`

*Direct* students, **in pairs**, to write a bash script called `countdown` containing a function that:
...
Just remember when you call the function, it's called from within the script itself. We don't pass arguments when executing the script. 
...
- **`$(( stop + 1 ))`** — **arithmetic expansion**. Bash won't do maths unless told to; without the double brackets, `stop + 1` is just text
  - Because it recognises it as an arithmetic expression we can't avoid the dollar symbol before `stop`
...
If we want to we can also pass arguments from outside the script. 

```bash
#!/bin/bash

# countdown - prints numbers from start down to stop, then a final word
# takes: $1 = start, $2 = stop, $3 = final message
...
It's similar to a function within a function in it's behaviour. 

- Run: `countdown 10 0 "Blastoff"`
...
- Run: `touch heredoc` and `chmod +x heredoc`
...
- `> index.js` — redirect that output into this file
...
Notice that in our **index.js** we also have a dollar symbol for the PORT variable. 
...
**`set -e`** — "if **any** command in this script fails, stop the whole script right there." 
...
It's basically providing a default paramter like we can do with JavaScript functions. 
...
**This is where the heredoc quoting difference matters**:
...
**ASK** <br>
Look at part 5. `connect.js` is quoted, `setup.js` is not. Why the difference? <br>
**ANSWER** <br>
...
**HANDS ON (45 min)** <br>
I want you to read through this script, familiarise yourself with it. Use Google, research the different elements. 

Once you're happy think about how you could extend this script. I'd copy it and make a new one, then play around.

- Could you add a `.env` file or a `.gitignore`?
